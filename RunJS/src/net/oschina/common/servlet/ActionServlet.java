...
import net.oschina.common.db.DBException;
...
public final class ActionServlet extends HttpServlet {
	...
	protected void handleDBException(RequestContext req, DBException e) throws ServletException, IOException {
		...
		SmtpHelper.reportError(req.request(), e);
	}
	...
	protected void handleException(RequestContext req, String msg) throws ServletException, IOException {
		...
		if(StringUtils.isNotBlank(ep)){
			...
			if(ep.charAt(0)!='/'){
				...
				//此处是为了使用使用filter做urlmapping 的方式
				req.request().setAttribute(ACTION_FORWARD, ep.trim());			
				//req.forward(ep.trim());
			}
		}
		else{
					...
					InvocationTargetException
	{
		...
		if(action instanceof ActionCallback) 
			((ActionCallback)action).beforeCall(m_action, req);		
		...
		if(action instanceof ActionCallback) 
			((ActionCallback)action).afterCall(m_action, req);		
